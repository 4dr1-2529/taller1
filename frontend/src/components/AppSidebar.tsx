...
export function AppSidebar({ sections, activeSection, onSelect, alertCount }: AppSidebarProps) {
  ...
  return (
    <aside className="w-full border-r border-slate-200/20 bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 text-slate-100 lg:min-h-screen lg:w-72 dark:border-slate-700/30">
      ...
      <div className="mt-auto border-t border-slate-800 p-4 space-y-3">
        ...
        {user ? (
          <div className="rounded-lg bg-white/5 p-3">
            <p className="text-xs font-medium text-white">{user.nombres} {user.apellidos}</p>
            <p className="text-[11px] text-slate-400 mt-0.5">
              <span className="capitalize">{roleLabel[user.role] ?? user.role}</span>
              {isAdmin && <span className="ml-1 text-indigo-300">· Admin</span>}
            </p>
          </div>
        ) : null}
