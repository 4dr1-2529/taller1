...
**Leyenda de estado:** ✅ Verificado (ejecutado con evidencia) · 📋 Planificado (no ejecutado en la auditoría 2026-09-26).
...
flowchart TB
  ...
  subgraph PROCESOS["PROCESOS"]
    ...
    P3[Feature engineering<br/>7 variables tesis]
...
| PR-05 | **Extracción de features** | Consolidar 7 variables para ML | `predict.controller` + queries Prisma |
...
| **KPI-06** F1 del modelo IA | F1 sobre el conjunto de **validación** (criterio de selección V6; el holdout no se usa para elegir) | ≥ 0.80 — **no alcanzado** | `machine-learning/artifacts/synthetic/metrics.json`: `best_f1_score=0.4333`, holdout F1=0.4286, `experimental=true` |
...
| **8.1 Planificación y control operacional** | Migraciones, repair y validación de datos (los seeds de población están deshabilitados) |
...
| ISO 9001 | 7.5 — Información documentada | Matrículas | `frontend/src/components/views/EnrollmentsView.tsx` | EN-02 `POST /matriculas` | `plan-pruebas/evidencias-finales/api/matricula-post-201-aislado-20260926.json` (HTTP 201, BD aislada) | ✅ Verificado |
...
| ISO 9001 | 8.1 — Control operacional | Persistencia | `backend/prisma/schema.prisma` | PR-03 57 modelos Prisma (54 activos + 3 `@@ignore`) | `npm run db:count-models` (2026-09-26) | ✅ Verificado |
...
| ISO 9001 | 9.1 — Seguimiento y medición | Predicción IA | `machine-learning/train.py` | PR-06 Stacking V6 (bases RF+XGB, meta-RF) | `artifacts/synthetic/metrics.json`: best_f1 0.4333 (validación), holdout F1 0.4286, `experimental=true` | ✅ Verificado |
...
| ISO 9001 | 6.2 — Objetivos de calidad | KPI cobertura notas | `backend/scripts/validate-demo-data.mjs` | KPI-03 19 680 calificaciones demo | Salida histórica del seed demo, no re-ejecutada el 2026-09-26 | 📋 Planificado |
