...
export function PredictionView({ students }: PredictionViewProps) {
  ...
  const base = useMemo(() => {
    ...
    return computePrediction(student.metrics, student.estado);
  }, [student]);
  ...
  const simulated = useMemo(() => {
    if (!student) return null;
    return simulateScenario(student.metrics, student.estado, scenarioDeltas);
      ...
      <section className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <h3 className="flex items-center gap-2 text-base font-semibold text-slate-900">
              ...
              Módulo de predicción (ensemble simulado)
            </h3>
            <p className="mt-1 max-w-2xl text-sm text-slate-600">
              El score (0–100) agrega factores académicos, asistencia, LMS y tareas. La versión final puede
              sustituirse por probabilidades calibradas desde <strong>ml-service</strong> (Python).
            </p>
          </div>
          <select
        ...
        <div className="mt-6 grid gap-4 lg:grid-cols-2">
          ...
          <article className="rounded-xl border border-indigo-100 bg-indigo-50/50 p-4">
            ...
            <p className="mt-2 text-sm text-indigo-950/90">{base.meta.descripcion}</p>
            <ul className="mt-3 list-disc space-y-1 pl-4 text-sm text-indigo-950/90">
              {base.meta.notas.map((n) => (
                <li key={n}>{n}</li>
              ))}
            </ul>
      ...
      <section className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-base font-semibold text-slate-900">Prueba de API Next.js</h3>
              <p className="text-sm text-slate-600">
                Endpoint <code className="rounded bg-slate-100 px-1 py-0.5 text-xs">POST /api/predict</code> —
                mismo núcleo que el cliente; preparado para delegar en <strong>ml-service</strong>.
              </p>
            </div>
          </div>
          <button
