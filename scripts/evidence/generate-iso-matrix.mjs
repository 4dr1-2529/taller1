...
const content = `# Matriz ISO 25010 — Evidencias visuales locales

Generado: ${new Date().toISOString()} · Entorno **local** (sin Railway/Vercel)

| Archivo evidencia | Característica ISO 25010 | Qué demuestra |
|-------------------|--------------------------|---------------|
${rows.map(([f, iso, desc]) => `| ${f} | ${iso} | ${desc} |`).join("\n")}
`;

const dest = path.join(OUT, "iso", "MATRIZ-ISO-25010.md");
fs.mkdirSync(path.dirname(dest), { recursive: true });
fs.writeFileSync(dest, content, "utf8");
console.log("  OK iso/MATRIZ-ISO-25010.md");
